...
make distclean
# BusyBox's defconfig can prompt when new options are introduced. Feed empty
# answers and then normalize the resulting configuration non-interactively.
make defconfig </dev/null || true
# The config helper is generated by the BusyBox build and is not guaranteed to
# exist until after the kconfig host tools have been built. Avoid depending on
# it: write the required static option directly and normalize with olddefconfig.
